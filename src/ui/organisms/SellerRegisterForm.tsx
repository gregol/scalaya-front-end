/**
 * Seller/Supplier Registration Form
 *
 ...
 * Request: { email, password, firstName, lastName, businessName, phone? }
 * Success response (201): { "message": string, "supplier": { id, email, firstName, lastName, businessName, phone, status } }
 */
...
type FieldErrors = {
  [K in keyof SellerRegistrationData]?: string;
};
...
export function SellerRegisterForm() {
  ...
  const handleInputChange = (
    field: keyof SellerRegistrationData,
    value: string
  ) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (validationErrors.length > 0) {
      ...
      validationErrors.forEach(({ field, message }) => {
        errors[field as keyof SellerRegistrationData] = message;
      });
    ...
    try {
      ...
      const endpoint = `${apiBaseUrl}/api/supplier/register`;
      ...
      if (!response.ok && data) {
        ...
        if (
          ...
          validationError.error.details.forEach((detail) => {
            errors[detail.field as keyof SellerRegistrationData] =
              detail.message;
          });
          setFieldErrors(errors);
